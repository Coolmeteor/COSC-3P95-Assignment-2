...
def handle_client(client_socket, data_folder, use_error_handling, use_checksum):
    with tracer.start_as_current_span("handle_client"):
        # Receive compression setting from client
        compression_setting = client_socket.recv(1)
        use_compression = compression_setting == b"1"
...
def start_server(data_folder, use_error_handling = False, use_checksum = False):
    ...
    while True:
        ...
        client_handler = multiprocessing.Process(target=handle_client, args=(client, data_folder, use_error_handling, use_checksum))
...
if __name__ == "__main__":
    ...
    start_server("server_files", True, True)